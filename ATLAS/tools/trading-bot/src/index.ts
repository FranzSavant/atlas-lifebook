...
let lastOpenTime: bigint | null = null;
...
async function bootstrap(): Promise<void> {
  ...
  if (snap) {
    ...
    engine.setState(snap.payload as unknown as Parameters<typeof engine.setState>[0]);
    ...
    if (from !== null) {
      ...
      if (delta.length > 0) await parquet.appendBatch(delta);
    }
  ...
  for (let i = 0; i < recent.length; i++) closes[i] = recent[i]!.close;
...
function onClosedCandle(candle: import("./core/RingBufferSoA.js").Ohlcv): void {
  ...
  if (closesSinceSnapshot >= SNAPSHOT_EVERY) {
    ...
    if (recent.length > 0) parquet.appendBatch(recent).catch((e) => console.error("[Parquet]", e));
  ...
  reconcileTimer = setTimeout(async () => {
    ...
    try {
      ...
      if (delta.length > 0) {
        ...
        for (const c of delta) onClosedCandle(c);
      } else {
