...
export class PingPongBuffer {
  private bankA: RingBufferSoA;
  private bankB: RingBufferSoA;
  private active: "A" | "B" = "A";

  /** Linearized snapshot visible to readers after last swap. */
  private snapshotCloses: Float64Array;
  ...
  seedAll(candles: Ohlcv[]): void {
    this.bankA.clear();
    this.bankB.clear();
    for (const c of candles) this.bankA.push(c);
    ...
    for (const c of candles) this.bankB.push(c);
    void n;
  }
   ...
   */
  swap(): { closes: Float64Array; count: number; newestTime: bigint | null } {
    ...
    if (newActive.count !== snapshotBank.count) {
      ...
      for (let i = 0; i < this.snapshotCount; i++) {
        ...
        if (newActive.newestTime !== c.openTime) {
          ...
          newActive.clear();
          for (let j = 0; j < this.snapshotCount; j++) newActive.push(snapshotBank.get(j));
          break;
        }
      }
    }
  ...
  getSnapshot(): { closes: Float64Array; count: number } {
