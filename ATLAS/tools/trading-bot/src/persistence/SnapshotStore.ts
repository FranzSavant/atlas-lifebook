...
export interface SnapshotPayload {
  ...
  newestOpenTime: string; // bigint as string
  count: number;
  head: number;
  watermarks: unknown;
  indicators: unknown[]; // IncrementalSnapshot[]
}
...
export class SnapshotStore {
  ...
  private checksum(obj: unknown): string {
    const json = JSON.stringify(obj);
    return createHash("sha256").update(json).digest("hex");
  }

  save(payload: Omit<SnapshotPayload, "schemaVersion" | "timestamp"> & { newestOpenTime: string }): string {
    const full: SnapshotPayload & { checksum?: string } = {
  ...
  loadLatest(): { payload: SnapshotPayload; checksumValid: boolean } | null {
    const files = readdirSync(this.dir).filter((f) => f.startsWith("snapshot-") && f.endsWith(".json")).sort().reverse();
    for (const f of files) {
      const p = join(this.dir, f);
      try {
        const raw = readFileSync(p, "utf-8");
        const obj = JSON.parse(raw) as SnapshotPayload & { checksum: string };
        ...
        if ((rest as SnapshotPayload).schemaVersion !== SCHEMA_VERSION) {
          // console.warn(`[Snapshot] schema mismatch, skipping`);
          continue;
        }
        return { payload: rest as SnapshotPayload, checksumValid: valid };
      } catch {
        continue;
      }
    }
  ...
  private prune(): void {
    ...
    for (let i = 0; i < files.length - keep; i++) {
      try { unlinkSync(join(this.dir, files[i]!)); } catch {}
    }
  }
