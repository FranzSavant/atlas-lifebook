...
for item in "${MANIFEST[@]}"; do
  if [ -d "$item" ]; then
    ...
    (cd "$item" && find . -type f | grep -v -- "-Statement.md" | grep -v "/logs/" | grep -v "/Procesado/" | grep -v "/Algún día/\|/Referencia/\|/Papelera/\|/Incubar/\|/Proyectos/\|/Próximas acciones/\|/Calendario/\|/Flash/\|/Delegadas/" | while read -r f; do
